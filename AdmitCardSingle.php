...
function my_simple_crypt( $string, $action = 'e' ) {
    ...
    return $output;
                    ...
                    $reg_no=my_simple_crypt($_GET['regno'],'d');
                    if(empty($id) || empty($reg_no)){
                        echo "<h1>Invalid ID AND REG NO</h1>";
                    }else{
                        $tt_query=mysqli_query($conn,"SELECT * FROM time_table where test_id='$id' ");
                        $tt=mysqli_fetch_array($tt_query);
                        $class=$tt['class'];
                    $students_query=mysqli_query($conn,"SELECT * FROM student_data where reg_no='$reg_no' order by section,student_name ASC ");
